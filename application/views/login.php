
<div class="container">
    <form action="./Welcome/session" method="POST" class="form-signin" style="margin-top:-50px">
        <div align="center">
          <img class="mb-4" src="https://getbootstrap.com/docs/4.1/assets/brand/bootstrap-solid.svg" alt="" width="72" height="72">
        </div>
        <label style="color:#bdc3c7">Iniciar Sesión</label>
        <input type="text" id="inputEmail" class="form-control" name="username"  placeholder="Usuario" required autofocus>
        <input type="password" id="inputPassword" class="form-control" name="password" placeholder="Contraseña" required>
        <button class="btn btn-primary btn-block" type="submit">Iniciar Sesión</button>
        <hr/>
          <div align="right" style="margin-top: -10px"><label style="color:#bdc3c7;font-size: 17px;cursor: pointer">Olvide mi contraseña</label></div>
      </form> 
</div>